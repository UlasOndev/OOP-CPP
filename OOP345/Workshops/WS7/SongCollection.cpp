...
namespace seneca {
	...
	void SongCollection::display(std::ostream& out)const {
		...
		mins = (i / 60) % 60;
		secs = i % 60;
		out << "| " << std::setw(77) << std::setfill(' ') << " Total Listening Time: " << hours << ":" << mins << ":" << secs << " |" << std::endl;
	}
	std::ostream& operator<<(std::ostream& out, const Song& theSong) {
		...
		if (theSong.s_year) out << theSong.s_year;
		else out << "";
	...
	void SongCollection::sort(const std::string& str) {
		auto compare = [str](const Song songA, const Song songB){
				bool check = false;
				if (str == "length") check = songA.s_length < songB.s_length;
				else if (str == "title") check = strcmp(songA.s_title.c_str(), songB.s_title.c_str()) < 0;
				else if (str == "album") check = strcmp(songA.s_album.c_str(), songB.s_album.c_str()) < 0;
				else{
	...
	void SongCollection::cleanAlbum() {
		auto cleanAlbum = [](Song& src){
				if (src.s_album == "[None]") src.s_album = "";
			};
	...
	std::list<Song> SongCollection::getSongsForArtist(const std::string name) const {
		...
		auto addSong = [name, &songList](const Song src){
				if (src.s_artist == name) songList.push_back(src);
			};
